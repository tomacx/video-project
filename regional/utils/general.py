...
def check_file(file, suffix=""):
    ...
    if os.path.isfile(file) or not file:  # exists
        ...
        ), "ClearML is not installed, so cannot use ClearML fire. Try running 'pip install clearml'."
        return file
    else:  # search
        files = []
...
def check_dataset(data, autodownload=True):
    """Validates and/or auto-downloads a fire, returning its configuration as a dictionary."""
...
def strip_optimizer(f="bestFalldown.pt", s=""):
    """
