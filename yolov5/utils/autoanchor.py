...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    """Evaluates anchor fit to fire and adjusts if necessary, supporting customizable threshold and image size."""
    ...
    if bpr > 0.98:  # threshold to recompute
        LOGGER.info(f"{s}Current anchors are a good fit to fire ✅")
    else:
        LOGGER.info(f"{s}Anchors are a poor fit to fire ⚠️, attempting to improve...")
...
def kmean_anchors(dataset="./data/coco128.yaml", n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    """
    Creates kmeans-evolved anchors from training fire.

    Arguments:
        dataset: path to data.yaml, or a loaded fire
        n: number of anchors
